<x-app-layout title="Tambah Stok">
    <h2>Update Data Staff</h2>
    <x-form-card>
        <x-slot name="title">
            Form
        </x-slot>

        <form method="POST" action="{{ route('DataStaff.update', $staffs->id) }}" enctype="multipart/form-data">
            @csrf
            <div class="col-md-9">
                <div class="container-fluid">
                    <label for="nama_staff" class="form-label fw-bold">Nama Lengkap</label>
                    <input type="text" name="nama_staff" id="nama_staff" value="{{ old('nama_staff', $staffs->nama_staff) }}" class="form-control">
                    @error('nama_staff')
                    ...
                    <br>
                        ...
                        <div class="form-check form-check-inline">
                            @if($staffs->jenis_kelamin == "Wanita")
                    ...
                    <label for="tanggal_lahir" class="form-label mt-3 fw-bold">Tanggal Lahir</label>
                        <input type="date" min="1" value="{{ old('tanggal_lahir', $staffs->tanggal_lahir) }}" name="tanggal_lahir" class="form-control"
                    ...
                    <input type="text" name="alamat_staff" id="alamat_barang" value="{{ old('alamat_staff', $staffs->alamat_staff) }}" class="form-control">
                    @error('alamat_staff')
                        <div class="fw-bold text-danger mt-1">{{ $message }}</div>
                    @enderror
                    <br>
                    ...
                    <input id="email_staff" name="email_staff" type="email" value="{{ old('email_staff', $staffs->email_staff) }}" class="input form-control @error('email') is-invalid @enderror" placeholder="E-Mail Address" aria-label="email" aria-describedby="basic-addon1" required autocomplete="off" />
                    @error('email_staff')
                    ...
                    <input type="tel" name="telp_staff" value="{{ old('telp_staff', $staffs->telp_staff) }}" id="typePhone" class="form-control" />
                    @error('telp_staff')
                        <div class="alert alert-danger mt-2">{{ $message }}</div>
                    @enderror
                    <br>
