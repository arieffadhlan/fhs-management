<x-app-layout title="Penambahan Data Penjualan Staff">
    ...
    <x-form-card>
        ...
        <form method="POST" action="{{ route('penjualanStaff.store') }}" enctype="multipart/form-data"
            class="row g-3">
            @csrf
            <div class="col-md-9">
                <div class="container-fluid">
                    <label class="form-label fw-bold">Nama Staff<sup style="color: red">*</sup></label>
                    <select name="staff_id" value="{{ old('staff_id') }}" class="form-select form-select-sm"
                    ...
                    <select name="nama_barang" value="{{ old('nama_barang') }}" class="form-select form-select-sm"
                        aria-label=".form-select-sm">
                        <option value="" selected disabled>Pilih barang</option>
                        @foreach ($stocks as $stock)
                            <option value="{{ $stock->nama_barang }}"
                                {{ old('nama_barang') == $stock->nama_barang ? 'selected' : '' }}>
                                {{ $stock->nama_barang }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <input type="number" min="1" value="{{ old('jumlah_penjualan') }}" name="jumlah_penjualan"
                        class="form-control" id="jumlah_penjualan">
                    ...
                    <input type="date" name="tanggal_penjualan" value="{{ old('tanggal_penjualan') }}"
